...
import { VSMBlurShader, SSSBlurShader, TinyBlurHDREShader, DOFShader } from "./shader_parameters.js"
...
let debugMode = QueryString.debug && "0" !== QueryString.debug && "false" !== QueryString.debug;
...
class VSMMaterial extends THREE.ShaderMaterial {
    constructor(use_float_texture) {
        ...
        super({ vertexShader: ShaderLibrary.get("shadow_vertex"), fragmentShader: t + ShaderLibrary.get("vsm_fragment") });
...
class Signal {
    ...
    dispatch(e) {
        for (var t = this._listeners.length, r = 0; r < t; ++r) this._listeners[r](e);
    }
    get hasListeners() {
...
class Skybox extends THREE.Mesh {
    constructor(env_map, width, exposure_exp, hdre) {
        ...
        box.scale(-1, 1, 1);
        var material = new SkyMaterial({ envMap: env_map, exposure: exposure_exp || 0, hdre: hdre });
        super(box, material);
    }
    get texture() {
...
class SkyMaterial extends THREE.ShaderMaterial {
    constructor(parameters) {
        ...
        super({ uniforms: uniforms, defines: defines, vertexShader: ShaderLibrary.get("sky_vertex"), fragmentShader: ShaderLibrary.get("sky_fragment") });
...
class OrbitController extends Component {
    ...
    onAdded() {
        ...
        this._container.addEventListener(e, this._onMouseWheel),
            ...
            this._moveWithKeys && (document.addEventListener("keyup", this._onKeyUp), document.addEventListener("keydown", this._onKeyDown));
    }
    onRemoved() {
        var e = /Firefox/i.test(navigator.userAgent) ? "DOMMouseScroll" : "mousewheel";
        this._container.removeEventListener(e, this._onMouseWheel),
            ...
            this._moveWithKeys && (document.removeEventListener("keyup", this._onKeyUp), document.removeEventListener("keydown", this._onKeyDown));
    }
    onUpdate(e) {
        ...
        var r = this.entity,
            n = this._m,
            a = this._fromSphericalCoordinates(this._coords.z, this._coords.x, this._coords.y);
        a.add(this.lookAtTarget), n.lookAt(a, this.lookAtTarget, new THREE.Vector3(0, 1, 0)), n.setPosition(a), n.decompose(r.position, r.quaternion, r.scale);
    }
    ...
    _initListeners() {
        ...
        (this._onMouseWheel = function (t) {
            ...
            (this._onTouchDown = function (t) {
                if (((e._oldMouseX = void 0), (e._oldMouseY = void 0), 2 === t.touches.length)) {
                    var r = t.touches[0],
                        n = t.touches[1],
...
class EntityEngine {
    ...
    update(time_delta) {
        ...
        for (var t = this._updateableEntities, r = t.length, n = 0; n < r; ++n) t[n]._update(time_delta);
    }
}
let EntityPrototype = {
    _init: function (e) {
...
let Entity = {
    ...
    addComponents: function (entity, component_list) {
        for (var r = 0; r < component_list.length; ++r) Entity.addComponent(entity, component_list[r]);
    },
    removeComponents: function (entity, component_list) {
        for (var r = 0; r < component_list.length; ++r) Entity.removeComponent(entity, component_list[r]);
    },
    ...
    removeComponent: function (entity, component) {
        ...
        (entity._components = 0 === a ? null : i), (component._entity = null), entity._updateRequiresUpdates(r);
    },
};
...
class AssetLibrary {
    constructor(e) {
        (this._numLoaded = 0), (this._queue = []), (this._assets = {}), e && "/" != e.charAt(e.length - 1) && (e += "/"), (this._basePath = e || ""), (this._onComplete = new Signal()), (this._onProgress = new Signal());
    }
    static Type = { JSON: 0, MODEL: 1, TEXTURE_2D: 2, TEXTURE_CUBE: 3, PLAIN_TEXT: 4 }
    get onComplete() {
    ...
    _json(e, t) {
        ...
        n.overrideMimeType("application/json"),
            ...
            (n.onreadystatechange = function () {
                4 === n.readyState && "200" == n.status && ((r._assets[t] = JSON.parse(n.responseText)), r._onAssetLoaded());
            }),
            n.send(null);
    }
    _plainText(e, t) {
        ...
        n.overrideMimeType("application/json"),
            n.open("GET", e, true),
            (n.onreadystatechange = function () {
                4 === n.readyState && "200" == n.status && ((r._assets[t] = n.responseText), r._onAssetLoaded());
            }),
            n.send(null);
    }
    _textureCube(e, t) {
        ...
        (this._assets[t] = n.load([e + "posX.jpg", e + "negX.jpg", e + "posY.jpg", e + "negY.jpg", e + "negZ.jpg", e + "posZ.jpg"], function () {
            r._onAssetLoaded();
        })),
    ...
    _texture2D(filename, name, r) {
        ...
        (texture.wrapS = THREE.RepeatWrapping), (texture.wrapT = THREE.RepeatWrapping), (texture.minFilter = THREE.LinearMipMapLinearFilter), (texture.magFilter = THREE.LinearFilter), window.highPerformance && (texture.anisotropy = 16);
    }
    _model(e, t, r) {
        var n = this,
            a = new r();
        (a.options = a.options || {}),
            ...
            a.load(e, function (e) {
                (n._assets[t] = e.children[0].geometry),
                    n._onAssetLoaded();
            });
    }
    _onAssetLoaded() {
        this._onProgress.dispatch(this._numLoaded / this._queue.length), ++this._numLoaded === this._queue.length ? this._onComplete.dispatch(this) : this.load();
    }
}
...
class LinearDepthMaterial extends THREE.ShaderMaterial {
    constructor() {
        super({ uniforms: { cameraNear: { value: 0 }, rcpCameraRange: { value: 0 } }, vertexShader: ShaderLibrary.get("linear_depth_vertex"), fragmentShader: ShaderLibrary.get("linear_depth_fragment") });
    };
    get cameraNear() {
...
class SceneDepthRenderer {
    constructor(scene, camera, renderer, scale) {
        (this._renderer = renderer), (this._scene = scene), (this._camera = camera), (this._scale = scale || 1), (this._depthMaterial = new LinearDepthMaterial());
    }
    get texture() {
    ...
    resize(e, t) {
        (e = Math.ceil(e * this._scale)),
            ...
            (this._renderTarget && this._renderTarget.width === e && this._renderTarget.height === t) ||
            (this._renderTarget = new THREE.WebGLRenderTarget(e, t, { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, generateMipmaps: false, depthBuffer: true, stencilBuffer: false }));
    }
    render() {
...
class VSMShadowRenderer {
    constructor(scene, renderer, light, texture_size) {
        ...
        this._shadowMap = new DoubleBufferTexture(texture_size, texture_size, { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBAFormat, type: a });
        this._light = light;
        this._renderer = renderer;
        this._scene = scene;
...
class SSSProfile {
    ...
    addLayer(variance, color) {
        if (4 === this._layers.length) throw new Error("Doesn't support more than 4 layers!");
...
class SSSRenderer {
    constructor(scene, camera, renderer, depth_renderer, sss_material_options, scale) {
        ...
        this._renderTarget = new DoubleBufferTexture(1, 1, { type: this._floatTexType, minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, generateMipmaps: false, depthBuffer: true, stencilBuffer: false });
    ...
    resize(e, t) {
        ...
        this._renderTarget = new DoubleBufferTexture(r, n, { type: this._floatTexType, minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, generateMipmaps: false, depthBuffer: true, stencilBuffer: false });
    }
    render() {
...
class SSSMaterial extends THREE.ShaderMaterial {
    constructor(parameters) {
        ...
        super({ uniforms: uniforms_with_lights, defines: defines, lights: true, vertexShader: ShaderLibrary.get("sss_vertex"), fragmentShader: ShaderLibrary.getInclude("include_ggx") + ShaderLibrary.get("sss_fragment") });
...
class DepthOfFieldPass extends Pass {
    ...
    set strength(e) {
        (this._strength = e), (this.composite.uniforms.strength.value = e), (this.enabled = 0 !== this._strength);
    }
    ...
    setSize(e, t) {
        if (((e = Math.floor(e)), (t = Math.floor(t)), this.resolutionX !== e || this.resolutionY !== t)) {
            ...
            var options = { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBAFormat, type: this._textureType },
                width = e >> 1,
                height = t >> 1;
...
export { CenteredGaussianCurve, FloatTex, DoubleBufferTexture, RectRenderer, Signal, Skybox, OrbitController, Entity, verifyExtension, VSMMaterial, AssetLibrary, SceneDepthRenderer, VSMShadowRenderer, SSSProfile, SSSRenderer, DepthOfFieldPass, isPlatformMobile, QueryString, debugMode, global_render_target_injector, RenderTargetInjector };
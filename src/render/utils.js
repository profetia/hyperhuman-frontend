...
let QueryString = (function () {
	...
	for (
		var e = {}, t = window.location.search.substring(1), r = t.split('&'), n = 0;
		n < r.length;
		n++
	) {
		var a = r[n].split('=')
		...
		;(e._components = []),
			...
			(e._updateRequiresUpdates = function (e) {
				e !== this._requiresUpdates &&
					((this._requiresUpdates = e), this._onRequireUpdatesChange.dispatch(this))
			})
	},
}
...
let Entity = {
	...
	addComponent: function (entity, component) {
		...
		Entity.convert(entity),
			...
			entity._updateRequiresUpdates(this._requiresUpdates || !!component.onUpdate),
...
class SSSMaterial extends THREE.ShaderMaterial {
	constructor(parameters) {
		...
		super({
			...
			fragmentShader:
				ShaderLibrary.getInclude('include_ggx') + ShaderLibrary.get('sss_fragment'),
		})
...
class DepthOfFieldPass extends Pass {
	...
	set focusRange(e) {
		;(this._focusRange = e),
			(this.composite.uniforms.focusRange.value = e / (this._camera.far - this._camera.near))
	}
	get strength() {
	...
	render(renderer, write_buffer, read_buffer, delta_time, mask_active) {
		this._v.copy(this._focusPosition),
			...
			(this.composite.uniforms.focusDepth.value =
				(-this._v.z - this._camera.near) / (this._camera.far - this._camera.near))

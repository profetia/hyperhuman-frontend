...
class SkinMaterial extends THREE.ShaderMaterial {
	constructor(parameters) {
		...
		var uniforms = {
			...
			roughnessMapRange: {
				value: void 0 === parameters.roughnessMapRange ? 0.5 : parameters.roughnessMapRange,
			},
			roughnessMedian: {
				value: void 0 === parameters.roughnessMedian ? 0.65 : parameters.roughnessMedian,
			},
		...
		super({
			...
			fragmentShader:
				ShaderLibrary.getInclude('include_beckmann') + ShaderLibrary.get('skin_fragment'),
		})
...
class SimpleThreeProject {
	...
	init(debug_mode, asset_library, options) {
		...
		this._options.preserveDrawingBuffer = this._options.preserveDrawingBuffer || false
...
class SSSSSContent {
	...
	initScene() {
		...
		this._skybox = new Skybox(this.assetLibrary.get('env_specular'), 3, this.probeExposure)
		...
		boundingbox.setFromObject(face_mesh), this.shadowRenderer.constrain(face_mesh)
	...
	resize(e, t) {
		...
		;(e *= r),
			(t *= r),
			this.fxaa && this.fxaa.uniforms.rcpRenderTargetResolution.value.set(1 / e, 1 / t),
	...
	update(time_delta) {
		...
		this.shadowsInvalid && (this.shadowRenderer.render(), (this.shadowsInvalid = false))
...
function startUp(mesh_profile) {
	...
	assetLibrary.queueAsset('model', mesh_profile.model, AssetLibrary.Type.MODEL, OBJLoader),
		assetLibrary.queueAsset('diffuse', mesh_profile.diffuse, AssetLibrary.Type.TEXTURE_2D),
		assetLibrary.queueAsset('normal', mesh_profile.normal, AssetLibrary.Type.TEXTURE_2D),

...
import { VSMBlurShader, SSSBlurShader, TinyBlurHDREShader, DOFShader } from "./shader_parameters_new.js"
...
let highPerformance = !isPlatformMobile() && !QueryString.lowPerformance;
...
class VSMHairMaterial extends THREE.ShaderMaterial {
    constructor(use_float_texture) {
        ...
        super({
            ...
            }, vertexShader: ShaderLibrary.get("shadow_vertex"), fragmentShader: t + ShaderLibrary.get("vsm_hair_fragment")
        });
        (this.extensions.derivatives = true);
...
class LinearHairDepthMaterial extends THREE.ShaderMaterial {
    constructor() {
        super({
            ...
            }, vertexShader: ShaderLibrary.get("linear_depth_vertex"), fragmentShader: ShaderLibrary.get("linear_hair_depth_fragment")
        });
        this.side = THREE.DoubleSide;
    };
    get cameraNear() {
...
class SceneDepthRenderer {
    ...
    resize(width, height) {
        ...
        if (!(this._renderTarget && this._renderTarget.width === width && this._renderTarget.height === height))
            (this._renderTarget = new THREE.WebGLRenderTarget(width, height, { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, generateMipmaps: false, depthBuffer: true, stencilBuffer: false }));

        if (!(this._renderTarget2 && this._renderTarget2.width === width && this._renderTarget2.height === height))
            (this._renderTarget2 = new THREE.WebGLRenderTarget(width, height, { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, generateMipmaps: false, depthBuffer: true, stencilBuffer: false }));
...
class HairMaterial extends THREE.ShaderMaterial {
    ...
    constructor(parameters) {
        ...
        var defines = { MIN_VARIANCE: -1e-4, LIGHT_BLEED_REDUCTION: 0.5, USE_TANGENT: true };
        var uniforms = {
        ...
        let uniforms_with_lights = THREE.UniformsUtils.merge([uniforms, THREE.UniformsLib.lights]);
        super({ uniforms: uniforms_with_lights, defines: defines, lights: true, vertexShader: ShaderLibrary.get("hair_vertex"), fragmentShader: ShaderLibrary.getInclude("include_beckmann") + ShaderLibrary.get("hair_fragment") });
...
let global_material_overrider = new MaterialOverrider();
let global_render_target_injector = new RenderTargetInjector();

export { CenteredGaussianCurve, FloatTex, DoubleBufferTexture, RectRenderer, Signal, Skybox, OrbitController, Entity, verifyExtension, VSMMaterial, SceneDepthRenderer, VSMShadowRenderer, SSSProfile, SSSRenderer, DepthOfFieldPass, isPlatformMobile, QueryString, debugMode, global_render_target_injector, RenderTargetInjector, key_dispose, highPerformance, HairMaterial, global_material_overrider };